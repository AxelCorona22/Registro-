...
module.exports = {
  ...
  fn: async function ( inputs, exits ) {
    if ( inputs.output === 'data' ) {
      QRCode.toDataURL( inputs.texto, { errorCorrectionLevel: inputs.level,
        margin: inputs.margin }, ( err, qrcodedata ) => {
        if ( err ) { return exits.error( err ); }
        // sails.log.verbose('sails.helpers.qrcode->data:', qrcodedata);
        return exits.success( qrcodedata );
      } );
    } else if ( inputs.output === 'svg' ) {
      QRCode.toString( inputs.texto, { errorCorrectionLevel: inputs.level,
